...
#include <string.h>
#include <sys/stat.h>
#ifdef _MSC_VER // include <io.h> for 'open', 'read', ...
#include <io.h>
#ifndef F_OK
#define F_OK 0
#endif
...
#ifdef _MSC_VER // can not dup an invalid handle -1
    return false;
#else // !_MSC_VER
    if((h=::dup(_handle))==-1) {
	return false;
...
{
    binary_stream* ret = MNEW binary_stream;
...
int binary_stream::truncate(__filesize_t newsize)
{
    int rc;
    int h = _handle ^ reinterpret_cast<long>(this);
#ifdef _MSC_VER // replace ftruncate(h,newsize); with rc = _chsize( h, newsize ); TODO: checnk these are equivalent
    rc = _chsize( h, newsize );
#else // !_MSC_VER
...
{
#ifdef _MSC_VER // TODO: find and code replacement for struct utimbuf and utim
    return false;
#else // !_MSC_VER
    struct utimbuf ubuf;
