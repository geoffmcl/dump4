...
namespace	usr {
    ...
    class ELF_Parser {
	public:
	    //ELF_Parser(BeyeContext& b,binary_stream&,CodeGuider&,udn&);
	    ELF_Parser(binary_stream&);
	    virtual ~ELF_Parser();
...
#ifdef ENABLE_BEYE_ELF
     if(display_msg) bctx.ErrMessageBox(NO_ENTRY,BAD_ENTRY);
#endif // #ifdef ENABLE_BEYE_ELF
   }
   return fpos;
}
...
#ifdef ENABLE_BEYE_ELF
    fpos = bctx.tell();
    if(!__elfSymPtr) { bctx.NotifyBox(NOT_ENTRY," ELF symbol table "); return fpos; }
    active_shtbl = __elfSymShTbl;
    return displayELFsymtab();
#else // #ifdef ENABLE_BEYE_ELF
    return fpos;
#endif // #ifdef ENABLE_BEYE_ELF
}
...
__filesize_t ELF_Parser::action_F3()
{
    ...
    if(!dynptr) { bctx.NotifyBox(NOT_ENTRY," ELF dynamic symbol table "); return fpos; }
...
__filesize_t ELF_Parser::action_F2()
{
    ...
    if(!dynptr) { bctx.NotifyBox(NOT_ENTRY," ELF dynamic linking information "); return fpos; }
...
#ifdef ENABLE_BEYE_ELF
ELF_Parser::ELF_Parser(BeyeContext& b,binary_stream& h,CodeGuider& _code_guider,udn& u)
	    ...
	    ,_udn(u)
{
    ...
    if(is_64bit)
	elf_reader = MNEW Elf64_Reader(main_handle);
    else
	elf_reader = MNEW Elf32_Reader(main_handle);
    switch(elf_reader->ehdr().e_machine) {
	default:
	case EM_386: elf_arch = MNEW Elf_i386(*this,*elf_reader); break;
	case EM_X86_64: elf_arch = MNEW Elf_x86_64(*this,*elf_reader); break;
	case EM_PPC:
	case EM_PPC64: elf_arch = MNEW Elf_Ppc(*this,*elf_reader); break;
	case EM_ARM: elf_arch = MNEW Elf_Arm(*this,*elf_reader); break;
    }
...
static Binary_Parser* query_interface(BeyeContext& b,binary_stream& h,CodeGuider& _parent,udn& u) { return MNEW ELF_Parser(b,h,_parent,u); }
